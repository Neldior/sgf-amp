        ...
        <div id="app">
            ...
            <?php
                ...
                if(isset($_GET)) {
                    ...
                    $lienJustificatifRepertoire = "../justif/";
                    //$idmission = 1;
                    ...
                    function missionEmploye($idmiss){
                       $pdo = connectbd(); //$pdo = connexionBase();
                       //preparation de la requette
                       $requete = "SELECT nom, prenom FROM employe emp INNER JOIN mission m ON emp.idemp = m.idemp WHERE idmission=$idmiss";
                       // execution de la requête 
                       $rq = $pdo->prepare($requete);
                       $rq->execute();
                       //recuperation du resultat de la requete
                       $result = $rq->fetch(PDO::FETCH_NUM);
                       echo "<h2> Mission n° ".$idmiss." de ".$result[0]." ". $result[1]."</h2>";
                    }
                    ...
                    function detailsMission($idmiss){
                       ...
                       $rq->execute();
                       $result = $rq->fetch(PDO::FETCH_NUM);
                       echo "<p> Mission du ".$result[0]." au ".$result[1]." à ".$result[3]."</p>";
                       echo "<p> Ordre de mission - justificatif disponible <span><a href=../om/".$result[2].">ici</a></span> </p>";
                    }
                    ...
                    function tabFraisReelMission($idmiss){
                       ...
                       $requete = "SELECT naturefrais, montanttotalfrais, j.nomfichier FROM justificatif j, detail_frais_reel df, associer_avance_detail ad, demande_avance da
                                   WHERE j.iddetailfrais = df.iddetailfrais AND df.iddetailfrais = ad.iddetailfrais AND ad.iddmdeavce = da.iddmdeavce AND da.idmission = $idmiss
                                   UNION

                                   SELECT naturefrais, montanttotalfrais , j.nomfichier FROM justificatif j, detail_frais_reel df, associer_rbmt_detail ar, demande_remboursement db
                                   WHERE j.iddetailfrais = df.iddetailfrais AND df.iddetailfrais = ar.iddetailfrais AND ar.iddmderbsmt = db.iddmderbsmt AND db.idmission = $idmiss ";
                       ...
                       foreach ($rq as $result) {
                            ...
                            $tabF['justificatif'] = $result["nomfichier"]; //$GLOBALS['lienJustificatifRepertoire'].$result["nomfichier"];
                    ...
                    function tabNuitee($idmiss){
                        $pdo = connectbd(); //connexionBase();
                        //preparation de la requete
                        $requete = "SELECT nbrenuitee, justificatifnuitee, tarifnuitee FROM demande_remboursement rb, mission m, zone_geographique z
                                    WHERE rb.idmission = m.idmission AND m.codezone = z.codezone AND rb.idmission = $idmiss ";
                        // execution de la requete
                        $rq = $pdo->prepare($requete); 
                        $rq->execute();
                        // recupération des données
                        ...
                        foreach ($rq as $result) {
                            ...
                            $tab['justificatif'] = $result["justificatifnuitee"]; //$GLOBALS['lienJustificatifRepertoire'].$result["justificatifnuitee"];
                            //$tabF['justificatif'] = $result["nomfichier"];
                    ...
                    function tabRepas($idmiss){
                        $pdo = connectbd(); //connexionBase();
                        //preparation de la requete
                        $requete = "SELECT nbrerepas, tarifrepas FROM demande_remboursement rb, mission m, zone_geographique z
                                    WHERE rb.idmission = m.idmission AND m.codezone = z.codezone AND rb.idmission = $idmiss ";
                        // execution de la requete
                        $rq = $pdo->prepare($requete); 
                        $rq->execute();
                    ...
                    function tabkm($idmiss){
                        $pdo = connectbd(); //connexionBase();
                        //preparation de la requete
                        $requete = "SELECT kmsparcourus, tarif FROM demande_remboursement d, voiture v, bareme_kilometrique b
                                WHERE d.immatriculation = v.immatriculation AND v.puissanceadmin = b.puissanceadmin AND d.idmission = $idmiss; ";
                        // execution de la requete
                        $rq = $pdo->prepare($requete); 
                        $rq->execute();
                        // recupération des données
                    ...
                    function TabFusionFraisMission($idmiss){
                        ...
                        if (!empty(tabNuitee($idmiss)) && tabNuitee($idmiss)['quantite']!=0){
                            array_push($tab,tabNuitee($idmiss));
                            //$tab = $tab+tabNuitee($idmiss);
                        }
                        if (!empty(tabRepas($idmiss)) && tabRepas($idmiss)['quantite']!=0){
                            array_push($tab,tabRepas($idmiss));
                            //$tab = $tab + tabRepas($idmiss);
                        }
                        if (!empty(tabkm($idmiss)) && tabkm($idmiss)['quantite']!=0){
                    ...
                    function listeFraisMission($idmiss){
                        ...
                        foreach ($tabFrais as $val) {
                            $dossier_justif="../justif/"; 
                            echo '<tbody>';
                            echo '   <tr class="text-center text-black">';
                            echo '       <td style="text-align: left;">'.mb_convert_case($val["natureFrais"], MB_CASE_UPPER, "UTF-8").'</td>';
                            echo '       <td style="text-align: center">'.$val["quantite"].'</td>';
                            echo '        <td style="text-align: right">'.$val["montant"].' &euro; </td>';
                            echo '       <td style="text-align: center"><a target="_blank" href="'.$dossier_justif.$val["justificatif"].'">Lien</a></td>'; //'.$val["justificatif"].'
                            echo '   </tr> </tbody>';
                        }
                        echo "<tfoot>";
                        echo '    <tr class="text-right bg-default text-white bolder">';
                        echo '    <td colspan="2" class="text-center"><strong>Montant Total :</strong></td>';
                        echo '    <td class="text-right"><strong>'.$mTotal.'&euro; </strong></td>';
                    ...
                    function etatAvance($idmiss){
                        $pdo = connectbd(); //connexionBase();
                        //preparation de la requete
                        $requete = "SELECT libelleetat FROM index_etat_avancement idx, etat_avcmt_avance et, demande_avance d, mission m
                                    WHERE idx.codeetat = et.codeetat AND et.iddmdeavce = d.iddmdeavce AND d.idmission = m.idmission AND m.idmission = $idmiss ";
                        // execution de la requete
                        $rq = $pdo->prepare($requete); 
                        $rq->execute();
                        $result = $rq->fetch(PDO::FETCH_NUM);
                    ...
                    function etatRemboursement($idmiss){
                        $pdo = connectbd(); //connexionBase();
                        //preparation de la requete
                        $requete = "SELECT libelleetat FROM index_etat_avancement idx, etat_avcmt_rbrsmt et, demande_remboursement d, mission m
                                    WHERE idx.codeetat = et.codeetat AND et.iddmderbsmt = d.iddmderbsmt  AND d.idmission = m.idmission AND m.idmission = $idmiss;";
                        // execution de la requete
                        $rq = $pdo->prepare($requete); 
                        $rq->execute();
                        $result = $rq->fetch(PDO::FETCH_NUM);
                                    ...
                                    <div class="card-header white">
                                        <div class="row">
                                            <div class="col-sm-12 text-left" style="margin-top: 5px;">
                                                <h4 class="blue-text"><strong><?/= $mode ?> Détails de la Mission N° <?= $idmission ?> </strong><strong class="text-black"> (Justificatif OM <a target="_blanck" href="<?= !empty($lienordremission) ? "../om/$lienordremission" : "#" ?>"> ici </a> )</strong></h4>
                                            </div>
                                        </div>
                                    </div>
                                                            ...
                                                            <?php 
                                                                ...
                                                                if (count($davce)!==0) {
                                                                    foreach ($davce as $avce) { $i++;
                                                                        ...
                                                                        $libEtat = $etatA['codeetat']==="V10"?"btn-warning":"btn-success";
                                                                        ?>
                                                                        <a href="#" onclick="fnLoadDetailsFraisDemande('avance',<?= $iddmdeavce; ?>);" style="cursor:pointer;">
                                                                            <tr class='text-black'>
                                                                                ...
                                                                                <td style="text-align: center"><a href="javascript:void(0)" class="btn <?= $libEtat ?> r-20"><strong><?= mb_strtoupper($etatA['libelleetat']); ?></strong></a></td>
                                                                            </tr>
                                                                        </a>
                                                                <?php } 
                                                                } ?>
                                                            ...
                                                            <?php 
                                                                ...
                                                                if (count($dremb)!==0) {
                                                                    foreach ($dremb as $remb) { $i++;
                                                                        ...
                                                                        $libEtat0 = $etatR['codeetat']==="V10"?"btn-warning":"btn-success";
                                                                        ?>
                                                                        <tr class='text-black'>
                                                                            <td style="text-align: center;"><?= $i; ?></td>
                                                                            ...
                                                                            <td style="text-align: center"><a href="javascript:void(0)" class="btn <?= $libEtat0 ?> r-20"><strong><?= mb_strtoupper($etatR['libelleetat']); ?></strong></a></td>
                                                                        </tr>
                                                                <?php } 
                                                                } ?>
                                                        </tbody>
                                                    ...
                                                    <table id="dtbl-example3" class="table table-bordered table-hover">
                                                        <thead class="bg-default text-white"> 
                                                            <tr class="text-center">
                                                                ...
                                                                <th style="width: 15%"><strong>Quantité</strong></th>
                                                                <th style="width: 15%"><strong>Montant</strong></th>
                                    ...
                                    <div class="col-md-6">
                                        <div class="card mb-3 shadow no-b r-0">
                                            ...
                                            <div class="card-body" style="margin: -10px 0 0 0">
                                                <div class="row">
                                                    <div class="col-sm-1 text-right" style="margin-top: -5px; margin-bottom: -5px"> </div>
                                                    <div class="col-sm-5 text-left" style="margin-top: 5px;">
                                                        <h5 class="text-black">Montant total de l'Avance : </h5>
                                                    </div>
                                                    <div class="col-sm-2 text-right" style="margin-top: 5px; margin-bottom: -5px"> 
                                                        <h5 class="text-black"><?= montantAvance($idmission)." &euro;" ?></h5></div>
                                                    <div class="col-sm-2 text-right" style="margin-top: -5px; margin-bottom: -5px"> 
                                                        <?php if (isset($etatA['libelleetat'])) {  $libEtat1 = $etatA['codeetat']==="V10"?"btn-warning":"btn-success"; ?>
                                                            <a href="javascript:void(0)" class="btn <?= $libEtat1 ?> r-20"><strong><?= mb_strtoupper($etatA['libelleetat']); ?></strong></a>
                                                        <?php } ?>
                                                    </div>
                                                </div>
                                                </br>
                                                ...
                                                <div class="row">
                                                    <div class="col-sm-1 text-right" style="margin-top: -5px; margin-bottom: -5px"> </div>
                                                    <div class="col-sm-5 text-left" style="margin-top: 5px;">
                                                        <h5 class="text-black">Montant de remboursement demandé : </h5>
                                                    </div>
                                                    <div class="col-sm-2 text-right" style="margin-top: 5px; margin-bottom: -5px"> 
                                                        <h5 class="text-black"><?= montantTotalFrais($idmission) - montantAvance($idmission)." &euro;" ?></h5></div>
                                                    <div class="col-sm-2 text-right" style="margin-top: -5px; margin-bottom: -5px"> 
                                                        <?php if (isset($etatR['libelleetat'])) { $libEtat2 = $etatR['codeetat']==="V10"?"btn-warning":"btn-success"; ?>
                                                            <a href="javascript:void(0)" class="btn <?= $libEtat2 ?> r-20"><strong><?= mb_strtoupper($etatR['libelleetat']); ?></strong></a>
                                                        <?php } ?>
                                                    </div>
                                                </div>
                                                </br></br></br>
                                                <!--<ul>                                                
                                                    <li> Montant total de l'Avance : </li> <?php //echo montantAvance($idmission)."&euro;" ?> 
                                                    <a href="#" class="btn btn-success r-20"><strong><?/= mb_strtoupper($etatA['libelleetat']); ?></strong></a>
                                                    <li> Montant de remboursement demandé : <?php //echo montantTotalFrais($idmission) - montantAvance($idmission)."&euro;" ?></li>
